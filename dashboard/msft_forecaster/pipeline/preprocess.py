...
# Expected columns for each raw table — used for schema validation
EXPECTED_COLUMNS = {
    "msft_daily": ["date", "open", "high", "low", "close", "volume"],
    "gold_prices": ["date", "gold_close"],
    "oil_prices": ["date", "oil_close"],
    "vix_data": ["date", "vix"],
}
...
class MSFTRecord(BaseModel):
    """Pydantic model for validating a single MSFT daily stock record."""
    ...
    def price_must_be_positive(cls, v: float) -> float:
        """Ensure all price fields are positive values."""
        if v <= 0:
    ...
    @classmethod
    def high_must_be_valid(cls, v: float, info) -> float:
        """Ensure high price is not below open or close."""
        return v
...
class ExternalRecord(BaseModel):
    """Pydantic model for validating gold, oil, or VIX records."""
    ...
    def value_must_be_positive(cls, v: float) -> float:
        """Ensure external indicator value is positive."""
        if v <= 0:
...
def validate_dataframe(
    ...
    """
    Validate all rows in a DataFrame using a Pydantic model.

    Rows that fail validation are logged and dropped.
    Valid rows are returned as a clean DataFrame.

    Args:
        df (pd.DataFrame): DataFrame to validate.
        model (type): Pydantic model class to validate against.
    ...
    for _, row in df.iterrows():
        try:
            ...
            invalid_count += 1
            logger.warning(
                f"{source_name} — invalid row dropped " f"(date={row.get('date', 'unknown')}): {e}"
            )

    if invalid_count > 0:
        logger.warning(f"{source_name}: dropped {invalid_count} invalid rows " f"out of {len(df)}")
    else:
        logger.info(f"{source_name}: all {len(df)} rows passed validation")
...
def validate_all_tables(
    ...
    """
    Validate all four raw data tables using Pydantic models.

    Each table is validated row by row. Invalid rows are logged and
    dropped before the data enters the pipeline.

    Args:
        msft_df (pd.DataFrame): MSFT daily stock data.
        gold_df (pd.DataFrame): Gold closing prices.
        oil_df (pd.DataFrame): Crude oil closing prices.
        vix_df (pd.DataFrame): VIX daily values.

    Returns:
        tuple: Validated msft_df, gold_df, oil_df, vix_df.
    """
    # rename value columns for validation
    gold_val = gold_df.rename(columns={"gold_close": "value"})
    oil_val = oil_df.rename(columns={"oil_close": "value"})
    vix_val = vix_df.rename(columns={"vix": "value"})

    msft_clean = validate_dataframe(msft_df, MSFTRecord, "msft_daily")
    gold_clean = validate_dataframe(gold_val, ExternalRecord, "gold_prices")
    oil_clean = validate_dataframe(oil_val, ExternalRecord, "oil_prices")
    vix_clean = validate_dataframe(vix_val, ExternalRecord, "vix_data")

    # rename back
    gold_clean = gold_clean.rename(columns={"value": "gold_close"})
    oil_clean = oil_clean.rename(columns={"value": "oil_close"})
    vix_clean = vix_clean.rename(columns={"value": "vix"})

    return msft_clean, gold_clean, oil_clean, vix_clean
...
def validate_schema(df: pd.DataFrame, table_name: str) -> None:
    """
    Verify that a DataFrame contains the expected columns for a given table.

    Raises ValueError immediately if any expected column is missing so the
    pipeline fails fast with a clear error rather than crashing mid-run.

    Args:
        df (pd.DataFrame): DataFrame to validate.
        table_name (str): Name of the source table for error messages.
    ...
    logger.info(f"{table_name}: schema valid — all expected columns present")
...
def load_all_tables(db_path: str = DB_PATH) -> tuple:
    ...
    # validate schema — check all expected columns are present
    validate_schema(msft_df, "msft_daily")
    validate_schema(gold_df, "gold_prices")
    validate_schema(oil_df, "oil_prices")
    validate_schema(vix_df, "vix_data")
...
def scale_features(
    ...
    logger.info(f"RobustScaler fitted on {split_idx} training rows. " f"Saved to {scaler_path}.")
    return df, scaler
...
def save_processed_features(
    ...
    conn = sqlite3.connect(db_path)
    df_to_save.to_sql("processed_features", conn, if_exists="replace", index=False)
    conn.close()
...
def run_pipeline(
    ...
    # step 2 — validate
    msft_df, gold_df, oil_df, vix_df = validate_all_tables(msft_df, gold_df, oil_df, vix_df)
    ...
    logger.info(f"Pipeline complete. " f"Train: {len(X_train)} rows, Test: {len(X_test)} rows.")
